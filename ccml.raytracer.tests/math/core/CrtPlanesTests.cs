...
namespace ccml.raytracer.tests.math.core
{
    public class CrtPlanesTests
    {
        ...
        {
            // Given p ← plane()
            var p = CrtFactory.Plane();
            // When n1 ← local_normal_at(p, point(0, 0, 0))
            var n1 = p.LocalNormalAt(CrtFactory.Point(0, 0, 0));
            // And n2 ← local_normal_at(p, point(10, 0, -10))
            var n2 = p.LocalNormalAt(CrtFactory.Point(10, 0, -10));
            // And n3 ← local_normal_at(p, point(-5, 0, 150))
            var n3 = p.LocalNormalAt(CrtFactory.Point(-5, 0, 150));
            // Then n1 = vector(0, 1, 0)
            Assert.IsTrue(n1 == CrtFactory.Vector(0,1,0));
            // And n2 = vector(0, 1, 0)
            Assert.IsTrue(n1 == CrtFactory.Vector(0, 1, 0));
            // And n3 = vector(0, 1, 0)
            Assert.IsTrue(n1 == CrtFactory.Vector(0, 1, 0));
        ...
        public void IntersectWithARayParallelToThePlane()
        {
            ...
            var r = CrtFactory.Ray(CrtFactory.Point(0, 10, 0), CrtFactory.Vector(0, 0, 1));
        ...
        public void ARayIntersectingAPlaneFromAbove()
        {
            ...
            var r = CrtFactory.Ray(CrtFactory.Point(0, 1, 0), CrtFactory.Vector(0, -1, 0));
            // When xs ← local_intersect(p, r)
            var xs = p.LocalIntersect(r);
            // Then xs.count = 1
