...
namespace ccml.raytracer.tests.math.core
{
    public class ViewTransformationsTests
    {
        ...
        public void TheTransformationMatrixForTheDefaultOrientation()
        {
            ...
            var from = CrtFactory.Point(0, 0, 0);
            // And to ← point(0, 0, -1)
            var to = CrtFactory.Point(0, 0, -1);
            // And up ← vector(0, 1, 0)
            var up = CrtFactory.Vector(0, 1, 0);
            // When t ← view_transform(from, to, up)
            var t = CrtFactory.ViewTransformation(from, to, up);
            // Then t = identity_matrix
            Assert.IsTrue(t == CrtFactory.IdentityMatrix(4,4));
        }
        ...
        public void AViewTransformationMatrixLookingInPositiveZDirection()
        {
            ...
            Assert.IsTrue(t == CrtFactory.ScalingMatrix(-1,1,-1));
        }
        ...
        public void TheViewTransformationMovesTheWorld()
        {
            ...
            var from = CrtFactory.Point(0, 0, 8);
            // And to ← point(0, 0, 0)
            ...
            Assert.IsTrue(t == CrtFactory.TranslationMatrix(0, 0, -8));
        }
        ...
        public void AnArbitraryViewTransformation()
        {
            ...
            var from = CrtFactory.Point(1, 3, 2);
            // And to ← point(4, -2, 8)
            var to = CrtFactory.Point(4, -2, 8);
            // And up ← vector(1, 1, 0)
            var up = CrtFactory.Vector(1, 1, 0);
            // When t ← view_transform(from, to, up)
            var t = CrtFactory.ViewTransformation(from, to, up);
