...
namespace chapter11.exercise.monogame
{
    class Program
    {
        ...
        private static async Task Render(int hSize, int vSize)
        {
            ...
            var patterns = new Func<CrtPattern>[]
            {
                ...
                () => CrtFactory.PatternFactory.ColorPerturbedPattern(
                    CrtFactory.PatternFactory.SolidColor(CrtFactory.CoreFactory.Color(0,0,0.8)),
                    (p,c) =>
                    {
            ...
            for (int j = 0; j < patterns.Length; j++)
            {
                ...
                for (int i = 0; i <= nbr; i++)
                {
                    floor.Material.Reflective = 0.60 + i * 0.40 / nbr;
                    //
                    middle.Material.Diffuse = i * 1.0 / nbr;
                    middle.Material.Specular = i * 1.0 / nbr;
                    middle.Material.Shininess = nbr * 10;
                    //
