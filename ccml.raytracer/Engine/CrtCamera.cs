...
namespace ccml.raytracer.Engine
{
    public class CrtCamera
    {
        ...
        internal CrtCamera(int hSize, int vSize, double fieldOfView)
        {
            ...
            //
            ViewTransformMatrix = CrtFactory.TransformationFactory.IdentityMatrix(4, 4);
        }
        ...
        public CrtRay RayForPixel(int px, int py)
        {
            ...
            var pixel = InverseViewTransformMatrix * CrtFactory.CoreFactory.Point(worldX, worldY, -1.0);
            var origin = InverseViewTransformMatrix * CrtFactory.CoreFactory.Point(0, 0, 0);
            var direction = ~(pixel - origin);
            return CrtFactory.EngineFactory.Ray(origin, direction);
        }
        ...
        {
            var image = CrtFactory.EngineFactory.Canvas(HSize, VSize);
